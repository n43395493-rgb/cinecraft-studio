...
/**
 * Global Helpers untuk Buka / Tutup Modal
 */
...
/**
 * Inisialisasi Event Modal & Event Delegation Universal
 */
function initModals() {
    ...
    // Delegasi Event Klik Global untuk Tutup Modal (Menangani tombol close, backdrop, & elemen di dalamnya)
    document.addEventListener('click', (e) => {
        const closeBtn = e.target.closest('.modal-close, .btn-close-modal, [data-close-modal]');
        const isBackdrop = e.target.classList.contains('modal-backdrop');

        if (closeBtn || isBackdrop) {
            e.preventDefault();
            const targetModal = e.target.closest('.modal-wrapper');
            if (targetModal) {
